...
public function update_projectedit_archive_data(Request $request){
 ...
 $status = \App\Project::where('id',$request->project_id)->update([
   ...
   public function get_all_project_for_calender(){
	...
	$i=0;
	foreach($all_project_ids as $all_project_id){
	   $temp_array[$i] =  \App\Project::select('project_name','id','project_color as colour')->where('company_id',$user->company_id)->where('id',
...
$all_project_id->project_id)->first();
   ...
   public function upload_new_task_image(Request $request) {
	...
	public function add_new_task(Request $request){
	  ...
	  if($request->task_list_id==1)
	  {
	  $folder = \App\FileFolder::select('id','folder_name','folder_type_id')->where('folder_name','internaltask')->where('folder_type_id','1')->where

('project_id',$request->source_id)->first();
	  }
	  else if($request->task_list_id==2)
	  {
	   $folder = \App\FileFolder::select('id','folder_name')->where('folder_name','externaltask')->where('folder_type_id','2')->where('project_id',

$request->source_id)->first();
	  }
	  // $folder = \App\FileFolder::create([
			   // 'project_id' 	=> $request->source_id,
			   // 'folder_name' 	=> $request->task_name,
			   // 'folder_type_id'=> $tasklists->task_type_id,
			   // 'created_by'	=> $user->id,
...
('project_files.source_type','project_task_file')->join('users','users.id','=','project_files.created_by')->get();
  ...
  public function get_project_discussion(Request $request){
   ...
   $role= \DB::table('user_roles')->select('role')->where('id',$user->userrole)->first();
	...
	if ($user_roles_type==1)
	{
$discusstions = \App\ProjectDiscussion::select('project_discussions.id','project_discussions.discussion_description','project_discussions.created_at','project_discussions.discussion_title','project_discussions.discussion_file_url','users.profile_image',DB::raw('CASE WHEN users.firstname = "" THEN users.email
         ELSE concat(firstname, " ", lastname) END AS `user_name`'))->where('discussion_project_id',$request->project_id)->where('archive','=',0)->join('users','users.id','=','project_discussions.created_by')->get();
	   $data = array();
...
ssions.discussion_file_url','users.profile_image',DB::raw('CASE
       ...
       END AS `user_name`'))->where('discussion_project_id',$request->project_id)->where('archive','=',0)->where
...
$results = $events->union($tasks)->union($meetings)->get();
   ...
   public function get_project_discussion_detail(Request $request){
	   ...
	   $user = JWTAuth::toUser($token);
        $role= \DB::table('user_roles')->select('role')->where('id',$user->userrole)->first();
		$user_role = $role->role;
	    $data = \App\ProjectDiscussion::select

('project_discussions.id','project_discussions.discussion_title','project_discussions.discussion_description','project_discussions.discussion_file_url','user

s.firstname','users.id as user_id','project_discussions.created_at','project_discussions.discussion_project_id')->where('project_discussions.id',$request->discussion_id)->join('users','users.id','=','project_discussions.created_by')->first();
   ...
   public function get_project_event(Request $request){
	 $token = JWTAuth::getToken(); 
	 $user = JWTAuth::toUser($token);  
	 $data2 = \App\Companysetting::where('company_id',$user->company_id)->first();
	$events = \DB::table('events')
                ->select(\DB::raw("event_title as title, event_start_date as start,event_end_date as end,'events' as event_type,id as event_id,'$data2-

>eventcolor' as color "))
				->where('source_type','project')
                ->where('source_id',$request->event_project_id);

	$tasks  = \DB::table('tasks')
                 ->select(\DB::raw("task_name as title, task_start_date as start,task_end_date as end,'tasks' as event_type,id as event_id,'$data2-
...
>taskcolor' as color "))
	...
	$meetings  = \DB::table('meetings')
                 ->select(\DB::raw("meeting_name as title, meeting_date as start,meeting_date as end,'meetings' as event_type,id as event_id,'$data2-

>meetingcolor' as color "))
                ->where('source_type','project')
                ->where('source_id',$request->event_project_id);
...
$results = $events->union($tasks)->union($meetings)->get();
   ...
   public function get_project_discussion_comment(Request $request){
	   ...
	   $comments = \App\ProjectDiscussionComment::select

('project_discussion_comments.id','project_discussion_comments.comment','project_discussion_comments.project_discussion_id','project_discussion_comments.pare

nt_comment_id','users.firstname','users.id as user_id','project_discussion_comments.created_at','users.profile_image')->where

('project_discussion_comments.project_discussion_id',$request->discussion_id)->where('project_discussion_comments.parent_comment_id','0')->leftjoin

('users','users.id','=','project_discussion_comments.created_by')->leftjoin('professional_details','professional_details.id','=','users.company_id')->get();
	   
	   foreach($comments as $comment){
		  
		    $child_comments = \App\ProjectDiscussionComment::select

('project_discussion_comments.id','project_discussion_comments.comment','project_discussion_comments.project_discussion_id','project_discussion_comments.pare

nt_comment_id','users.firstname','users.id as user_id','project_discussion_comments.created_at','users.profile_image')->where

('project_discussion_comments.parent_comment_id',$comment->id)->leftjoin('users','users.id','=','project_discussion_comments.created_by')->leftjoin

('professional_details','professional_details.id','=','users.company_id')->get();
...
('project_discussion_comments.parent_comment_id',$request->parent_comment_id)->leftjoin('users','users.id','=','project_discussion_comments.created_by')->leftjoin('professional_details','professional_details.id','=','users.company_id')->get();
		return new JsonResponse([
            'status_code' => 200,
			'message' => 'Comment_save',
			'data'	=>	$child_comments,
...
t.`task_owner_ids`) where `t`.`source_type`='project' and `t`.`source_id`='$request->project_id' and Date(`t`.`task_end_date`) = CURDATE() GROUP BY t.id") );
   ...
   public function update_task(Request $request){
    ...
    public function check_email_exist(Request $request){
		$token = JWTAuth::getToken(); 
	   	$user = JWTAuth::toUser($token);
		$data = \App\User::select('user_types.type')->where('email',$request->email)->join('user_types','user_types.id','=','users.usertype')->first();
		...
		else if(count($data) && $data->type !='Home Owner'){
			 return new JsonResponse([
				'status_code' => 200,
				'check' => false,
        	]);
